...
class _SeekerUpdateRequestPageState extends State<SeekerUpdateRequestPage> {
  ...
  @override
  void initState() {
    serviceType = SeekerRequestDetailState.request.service ?? "";
    titleTextController.text = SeekerRequestDetailState.request.title ?? "";
    descriptionTextController.text =
        SeekerRequestDetailState.request.description ?? "";
    categoryTextController.text = SeekerRequestDetailState.request.title ?? "";
    locController.text = SeekerRequestDetailState.request.locationAddress ?? "";
    if (SeekerRequestDetailState.request.isRangePrice!) {
      var prices = SeekerRequestDetailState.request.price!.split("-");
      minimumPriceTextController.text = prices[0];
      maximumPriceTextController.text = prices[1];
    } else {
      priceTextController.text = SeekerRequestDetailState.request.price ?? "";
    }
    context.read<SeekerBloc>().add(ChangeLocationEvent(change: false));
    context.read<SeekerBloc>().add(
      RequestPriceEvent(
        fixedPrice: !SeekerRequestDetailState.request.isRangePrice!,
      ),
    );
    super.initState();
  }
    ...
    return Scaffold(
      body: BlocConsumer<SeekerBloc, SeekerState>(
        listener: (context, state) {
          ...
          } else if (state is FailureUpdateRequestState) {
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    CustomTextWidget(text: "UPDATE REQUEST", fontSize: 19),
                    const SizedBox(height: 46.0),
                    InputDropdownWidget(
                      value: SeekerRequestDetailState.request.service!,
                      items: [
                        DropdownMenuItem(
                          value: "Developer",
                                      ...
                                      children: [
                                        ListTile(
                                          onTap: () {
                                            ...
                                            context.read<SeekerBloc>().add(
                                              ChangeLocationEvent(change: true),
                                            );
                                            Helpers.getLocation();
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        CustomRadioButton(
                          ...
                          defaultSelected:
                              (SeekerRequestDetailState.request.isRangePrice!)
                                  ? "PRICE RANGE"
                                  : "FIXED PRICE",
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child:
                            (!SeekerRequestDetailState.request.withImage!)
                                ? Center(
                                  child: Column(
                    ...
                    ButtonWithoutIconWidget(
                      ...
                      onPressed: () {
                        ...
                        Request request = Request(
                          id: SeekerRequestDetailState.request.id,
                          title: titleTextController.text.trim(),
                          description: descriptionTextController.text.trim(),
                          acceptedUsers:
                              SeekerRequestDetailState.request.acceptedUsers,
                          approved: SeekerRequestDetailState.request.approved,
                          done: SeekerRequestDetailState.request.done,
                          approvedUser:
                              SeekerRequestDetailState.request.approvedUser,
                          service: serviceType,
                          price:
                              (RequestPriceState.fixedPrice)
                          ...
                          latitude:
                              (RequestLocationChangeState.change)
                                  ? (UseMapState.useMap)
                                      ? MapLocationState.location.latitude
                                      : locationData.latitude
                                  : SeekerRequestDetailState.request.latitude,
                          longitude:
                              (RequestLocationChangeState.change)
                                  ? (UseMapState.useMap)
                                      ? MapLocationState.location.longitude
                                      : locationData.longitude
                                  : SeekerRequestDetailState.request.longitude,
                          withImage: (ImageSeekerState.picture != null),
                        );
                        if (key.currentState!.validate()) {
                          context.read<SeekerBloc>().add(
                            UpdateRequestEvent(request: request),
                          );
                        }
                      },
                    ),
